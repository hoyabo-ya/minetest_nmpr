  ...
  core::list<FastFace*> &dest )
{
  ...
  */
  ...
  v3f posRelative_f( getPosRelative().X, getPosRelative().Y, getPosRelative().Z ); // floating point conversion
      ...
      */
      if ( material0 != material1
        ...
        if ( material0 != MATERIAL_AIR ) {
          f = makeFastFace( material0, light, sp, face_dir_f, scale, posRelative_f );
        }
        // If node at sp is air
        else {
          f = makeFastFace( material1, light, sp + face_dir_f, -1 * face_dir_f, scale, posRelative_f );
        }
        dest.push_back( f );
      }
...
void MapBlock::updateFastFaces()
{
  ...
  */
  ...
  for ( s16 y = -1; y < MAP_BLOCKSIZE; y++ ) {
    for ( s16 z = 0; z < MAP_BLOCKSIZE; z++ ) {
      updateFastFaceRow( v3s16( 0, y, z ), MAP_BLOCKSIZE, v3s16( 1, 0, 0 ), v3s16( 0, 1, 0 ), *fastfaces_new );
    }
  }
  /*
  ...
  */
  ...
  for ( s16 x = -1; x < MAP_BLOCKSIZE; x++ ) {
    for ( s16 y = 0; y < MAP_BLOCKSIZE; y++ ) {
      updateFastFaceRow( v3s16( x, y, 0 ), MAP_BLOCKSIZE, v3s16( 0, 0, 1 ), v3s16( 1, 0, 0 ), *fastfaces_new );
    }
  }
  /*
  ...
  */
  ...
  for ( s16 z = -1; z < MAP_BLOCKSIZE; z++ ) {
    for ( s16 y = 0; y < MAP_BLOCKSIZE; y++ ) {
      updateFastFaceRow( v3s16( 0, y, z ), MAP_BLOCKSIZE, v3s16( 1, 0, 0 ), v3s16( 0, 0, 1 ), *fastfaces_new );
    }
  }
